...
modifier_ch_3_14 = class({})

--------------------------------------------------------------------------------

function modifier_ch_3_14:GetTexture()
	return "item_treasure/力量护腕"
end

function modifier_ch_3_14:IsHidden()
	return true
end
--------------------------------------------------------------------------------

function modifier_ch_3_14:OnCreated( kv )
	self:OnRefresh()
end
--------------------------------------------------------------------------------

function modifier_ch_3_14:OnRefresh()
	if IsServer() then
		self.nFXIndex = ParticleManager:CreateParticle( "particles/ch/shopmall_ch_3_14.vpcf", PATTACH_OVERHEAD_FOLLOW, self:GetParent() )
	--	ParticleManager:SetParticleControlEnt( self.nFXIndex, 0, self:GetParent(), PATTACH_OVERHEAD_FOLLOW, "follow_head", self:GetParent():GetAbsOrigin(), true )
		...
		netTable["tmz"] = netTable["tmz"] + 8
		netTable["xyz"] = netTable["xyz"] + 5
		SetNetTableValue("UnitAttributes",unitKey,netTable)
	end
end
function modifier_ch_3_14:OnDestroy()
	if IsServer() then
...
function modifier_ch_3_14:GetAttributes()
