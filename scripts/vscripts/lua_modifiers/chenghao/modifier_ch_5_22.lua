...
modifier_ch_5_22 = class({})

--------------------------------------------------------------------------------

function modifier_ch_5_22:GetTexture()
	return "item_treasure/力量护腕"
end

function modifier_ch_5_22:IsHidden()
	return true
end
--------------------------------------------------------------------------------

function modifier_ch_5_22:OnCreated( kv )
	self:OnRefresh()
end
--------------------------------------------------------------------------------

function modifier_ch_5_22:OnRefresh()
	if IsServer() then
		self.nFXIndex = ParticleManager:CreateParticle( "particles/ch/shopmall_ch_dy_2.vpcf", PATTACH_OVERHEAD_FOLLOW, self:GetParent() )
	--	ParticleManager:SetParticleControlEnt( self.nFXIndex, 0, self:GetParent(), PATTACH_OVERHEAD_FOLLOW, "follow_head", self:GetParent():GetAbsOrigin(), true )
		...
		netTable["tmz"] = netTable["tmz"] + 30
		netTable["bfbtsqsx"] = netTable["bfbtsqsx"] + 15
		netTable["tmz3"] = netTable["tmz3"] + 35
		netTable["jnmjts"] = netTable["jnmjts"] + 0.1
		SetNetTableValue("UnitAttributes",unitKey,netTable)
	end
end
function modifier_ch_5_22:OnDestroy()
	if IsServer() then
...
function modifier_ch_5_22:GetAttributes()
