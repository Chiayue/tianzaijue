	...
	for i=1,num do
		...
		ParticleManager:SetParticleControl(particleID2,0,position)
		 ...
		 TimerUtil.createTimerWithDelay(0.1,function()
			...
			CreateDyh(keys,position,ewsh,ewxl)
			end
			)
		end
		)	
...
function CreateDyh(keys,position,ewsh,ewxl)
	local caster = keys.caster
	local ability = keys.ability
	...
	local x = 1 + (level+caster.cas_table.grjndj) / 10
		...
		local damage = (sx * i + baseDamage2 ) * x  * ((caster.cas_table["zhwsh"]+100)/100)  * shbs*ewsh
		...
		if RollPercentage(20) then	--百分之二十的概率触发暴击伤害
				damage2 = damage * (caster.dyh_multiple + 1 )
