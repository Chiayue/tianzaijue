...
function LightningJump(keys)
	...
	local baseDamage2 = baseDamage + caster.lssd_baseDamage
	i = i + caster.lssd_damage
	...
	target:RemoveModifierByName("modifier_zljcjn_lssd1")
    ...
    function()
		...
		if ability.jump_count[current] > 0 then
			...
			if new_target ~= nil then
				...
				ability:ApplyDataDrivenModifier(caster, new_target, "modifier_zljcjn_lssd1", {})
			else
...
function NewInstance(keys)
	...
	ability.jump_count[ability.instance] = ability.jump_count[ability.instance] + caster.lssd_max
