...
function Spawn( entityKeyValues )
	...
	OneAbility = thisEntity:FindAbilityByName( "boss_tiny_attack_lua" )
	TwoAbility = thisEntity:FindAbilityByName( "boss_tiny_yell_lua" )
	ThreeAbility = thisEntity:FindAbilityByName( "boss_tiny_bloodfire_lua" )
	FourAbility = thisEntity:FindAbilityByName( "boss_tiny_throw_lua" )
	FiveAbility = thisEntity:FindAbilityByName( "boss_tiny_jump_lua" )
...
function BossthreeThink()
	...
	if OneAbility ~= nil and OneAbility:IsCooldownReady()  then
		return CastPoint( enemies[1],OneAbility)
	end
	if FiveAbility ~= nil and FiveAbility:IsCooldownReady()  and thisEntity:GetHealthPercent() < 75  then
		...
		return CastPoint( target,FiveAbility)
	end
	if TwoAbility ~= nil and TwoAbility:IsCooldownReady() and thisEntity:GetHealthPercent() < 50 then

		return CastNoTarget( TwoAbility)
	end
	if FourAbility ~= nil and FourAbility:IsCooldownReady() and thisEntity:GetHealthPercent() < 35 then

		return CastNoTarget( FourAbility)
	end
	if ThreeAbility ~= nil and ThreeAbility:IsCooldownReady() and thisEntity:GetHealthPercent() < 10 then
