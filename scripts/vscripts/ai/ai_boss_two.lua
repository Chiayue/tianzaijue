...
function Spawn( entityKeyValues )
	...
	OneAbility = thisEntity:FindAbilityByName( "boss_two_flash_lua" )
	TwoAbility = thisEntity:FindAbilityByName( "boss_two_endaoe_lua" )
	
	thisEntity.timeOfLastAggro = GameRules:GetGameTime()
	thisEntity:SetContextThink( "BossOneThink", BossOneThink, 1 )
end

function BossOneThink()
	...
	if OneAbility ~= nil and OneAbility:IsCooldownReady()  and thisEntity:GetHealthPercent() < 70 then
		return CastNoTarget( OneAbility)
	end
	if TwoAbility ~= nil and TwoAbility:IsCooldownReady() and thisEntity:GetHealthPercent() < 25 then
		return CastNoTarget( TwoAbility)
	end
	if ThreeAbility ~= nil and ThreeAbility:IsCooldownReady() and thisEntity:GetHealthPercent() < 20 then
