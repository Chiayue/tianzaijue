...
function Spawn( entityKeyValues )
	...
	OneAbility = thisEntity:FindAbilityByName( "boss_wood_lower_attr_lua" )
	TwoAbility = thisEntity:FindAbilityByName( "boss_wood_bot_lua" )
	ThreeAbility = thisEntity:FindAbilityByName( "boss_wood_bot_lunpan_lua" )
	FourAbility = thisEntity:FindAbilityByName( "boss_wood_charge_lua" )
	FiveAbility = thisEntity:FindAbilityByName( "boss_wood_bot_lunpan_end_lua" )
	
	thisEntity.timeOfLastAggro = GameRules:GetGameTime()
	thisEntity:SetContextThink( "BosssixThink", BosssixThink, 1 )
end

function BosssixThink()
	...
	if FiveAbility ~= nil and FiveAbility:IsCooldownReady() and thisEntity:GetHealthPercent() < 25 then

		return CastNoTarget(FiveAbility)
	end
	if ThreeAbility ~= nil and ThreeAbility:IsCooldownReady() and thisEntity:GetHealthPercent() < 40 then

		return CastNoTarget( ThreeAbility)
	end
	if TwoAbility ~= nil and TwoAbility:IsCooldownReady() and thisEntity:GetHealthPercent() < 60 then

		return CastNoTarget( TwoAbility)
	end
	if FourAbility ~= nil and FourAbility:IsCooldownReady() and thisEntity:GetHealthPercent() < 80  then
		local target=enemies[1]
		for _,enemy in pairs(enemies) do
	    		if ( ((thisEntity:GetOrigin() - target:GetOrigin()):Length2D())<(( thisEntity:GetOrigin() - enemy:GetOrigin() ):Length2D() ))then

	    			target=enemy
	    		end
	    		
	    	end
		return CastTarget(target,FourAbility)
