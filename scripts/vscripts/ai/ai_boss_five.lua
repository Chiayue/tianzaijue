...
function Spawn( entityKeyValues )
	...
	OneAbility = thisEntity:FindAbilityByName( "boss_phoenix_stone_lua" )
	TwoAbility = thisEntity:FindAbilityByName( "boss_phoenix_fireline_lua" )
	ThreeAbility = thisEntity:FindAbilityByName( "boss_phoenix_turnegg_lua" )
	FourAbility = thisEntity:FindAbilityByName( "boss_phoenix_firemana_lua" )
	FiveAbility = thisEntity:FindAbilityByName( "boss_phoenix_firetarget_lua" )
...
function BossthreeThink()
	...
	if ThreeAbility ~= nil and ThreeAbility:IsCooldownReady() and thisEntity:GetHealthPercent() < 15 then

		return CastNoTarget( ThreeAbility)
	end
	if TwoAbility ~= nil and TwoAbility:IsCooldownReady() and thisEntity:GetHealthPercent() < 30 then

		return CastNoTarget( TwoAbility)
	end
	if FourAbility ~= nil and FourAbility:IsCooldownReady()  then

		return CastNoTarget( FourAbility)
	end
	if FiveAbility ~= nil and FiveAbility:IsCooldownReady() and thisEntity:GetHealthPercent() < 90 then

		return CastTarget(enemies[1] ,FiveAbility)
	end
	
	if OneAbility ~= nil and OneAbility:IsCooldownReady() and thisEntity:GetHealthPercent() < 50 then
