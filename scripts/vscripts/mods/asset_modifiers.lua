...
function public:init(bReload)
	...
	if IsServer() then
		EventManager:register(ET_GAME.NPC_FIRST_SPAWNED, 'OnGameEvent_NpcFirstSpawned', self)
...
function public:GetSoundReplacement(sSoundName, hUnit)
	...
	local sUnitName = hUnit:GetUnitName()
	local tAssetModifiers = KeyValues.AssetModifiersKv and KeyValues.AssetModifiersKv[sUnitName]
	if tAssetModifiers ~= nil then
		for _, tAssetModifier in pairs(tAssetModifiers) do
...
if IsServer() then
	...
	--
	function public:OnGameEvent_NpcFirstSpawned(tEvents)
		-- MARK:
		-- local hSpawnedUnit = EntIndexToHScript(tEvents.entindex)
		-- if (IsValid(hSpawnedUnit) and hSpawnedUnit:IsHero() == false and KeyValues.UnitsKv[hSpawnedUnit:GetUnitName()] ~= nil) and
		-- (KeyValues.UnitsKv[hSpawnedUnit:GetUnitName()].AttackSpeedActivityModifiers ~= nil or
		-- KeyValues.UnitsKv[hSpawnedUnit:GetUnitName()].AttackRangeActivityModifiers ~= nil or
		-- KeyValues.UnitsKv[hSpawnedUnit:GetUnitName()].MovementSpeedActivityModifiers ~= nil) then
