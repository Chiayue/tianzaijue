...
ENEMY_ATTRIBUTE_MODIFIER = function(hUnit)
	...
	if hUnit:IsBoss() then
		...
		tBasePercentage[EMDF_STATUS_HEALTH_BONUS] = ({ 0, 150, 200, 350 })[iPlayerCount] + (400 / ({ 18, 18, 18, 18 })[iBoss])
		tBasePercentage[EMDF_PHYSICAL_ATTACK_BONUS] = ({ 0, 0, 0, 0 })[iPlayerCount] + 67
		tBasePercentage[EMDF_MAGICAL_ATTACK_BONUS] = ({ 0, 0, 0, 0 })[iPlayerCount] + 67
		tBasePercentage[EMDF_PHYSICAL_ARMOR_BONUS] = ({ 0, 0, 0, 0 })[iPlayerCount] + 67
		tBasePercentage[EMDF_MAGICAL_ARMOR_BONUS] = ({ 0, 0, 0, 0 })[iPlayerCount] + 67
		--年兽BOSS
	...
	elseif not hUnit:IsGoldWave() then
		-- 非宝箱怪
		-- 额外百分比
		tBasePercentage[EMDF_STATUS_HEALTH_BONUS] = 0 + 67
		tBasePercentage[EMDF_PHYSICAL_ATTACK_BONUS] = 0 + 33
		tBasePercentage[EMDF_MAGICAL_ATTACK_BONUS] = 0 + 33
